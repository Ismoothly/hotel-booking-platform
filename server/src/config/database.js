...
const dbConfig = {
  development: {
    url: process.env.MONGODB_URI || 'mongodb://localhost:27017/hotel-booking-dev',
    options: {
  ...
  test: {
    url: process.env.MONGODB_TEST_URI || 'mongodb://localhost:27017/hotel-booking-test',
    options: {
      useNewUrlParser: true,
      useUnifiedTopology: true,
      retryWrites: true,
...
async function connectDB() {
  try {
    ...
    console.log(`数据库 URI: ${connectionConfig.url.replace(/\/\/.*:.*@/, '//***:***@')}`);
